...
class DailyScheduleView extends StatefulWidget {
  ...
  const DailyScheduleView({Key? key, required this.filteredSchedules}) : super(key: key);
...
class _DailyScheduleViewState extends State<DailyScheduleView> {
  ...
  List<Map<String, dynamic>> filterSchedule() {
    ...
    if (_searchQuery.isNotEmpty) {
      formatSchedule = formatSchedule
          .where((schedule) =>
      schedule['course_name'].toString().contains(_searchQuery) ||
          schedule['professor_name'].toString().contains(_searchQuery))
          .toList();
    }
    ...
    if (selectedStatus != 'همه کلاس‌ها') {
      ...
      schedule['course_status'] ==
          (selectedStatus == 'فقط در حال برگزاری‌ها' ? 'فعال' : 'غیرفعال'))
          .toList();
    }

    // مرتب‌سازی بر اساس روز و زمان
    formatSchedule.sort((a, b) {
      int dayOrderA = _dayOrder[a['day_name']] ?? 5;
      int dayOrderB = _dayOrder[b['day_name']] ?? 5;
